...
test('JustWoker defaults to the JSON API and only migrates the obsolete built-in URL', async () => {
  await initDb()
  const initial = await dbGet("SELECT target_url FROM site_monitors WHERE source = 'justwoker_models'")
  assert.equal(initial.target_url, 'https://api.justwoker.icu/api/pricing')
  ...
  await dbRun(
    ...
    ['https://api.justwoker.icu/pricing', 'encrypted-placeholder'],
  )
  await closeDb()
  await initDb()

  const migrated = await dbGet("SELECT * FROM site_monitors WHERE source = 'justwoker_models'")
  assert.equal(migrated.target_url, 'https://api.justwoker.icu/api/pricing')
