...
async function loadConfig() {
  const rows = await new Promise((resolve, reject) => getDb().all("SELECT key, value FROM system_config WHERE key IN ('llm_url', 'llm_model', 'llm_key')", (err, data) => err ? reject(err) : resolve(data || [])))
  const config = Object.fromEntries(rows.map((row) => [row.key, row.value]))
  if (!config.llm_key) return null
  return { url: (config.llm_url || 'https://api.deepseek.com').replace(/\/$/, ''), model: config.llm_model || 'deepseek-chat', key: decrypt(config.llm_key) }
}
...
export async function reviewGithubRepository(repository) {
  ...
  const response = await fetch(targetUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + config.key }, body: JSON.stringify({ model: config.model, temperature: 0.1, messages: [{ role: 'user', content: prompt }] }) })
  ...
  const result = await dbRun('INSERT INTO github_ai_reviews (repository_id, provider, model, category, summary, confidence, worth_push, raw_output) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [repository.id, 'openai-compatible', config.model, String(parsed.category || '').slice(0, 40), String(parsed.summary || '').slice(0, 180), confidence, parsed.worthPush ? 1 : 0, content.slice(0, 10000)])
