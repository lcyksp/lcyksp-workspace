...
function normalizeMailbox(value, fallback = '') {
...
async function sendSmtpMessage({ to, subject, body, contentType = 'text/plain' }) {
  ...
  try {
    ...
    const message = [`From: <${c.from}>`, `To: <${recipient}>`, `Subject: =?UTF-8?B?${Buffer.from(subject).toString('base64')}?=`, 'MIME-Version: 1.0', `Content-Type: ${contentType}; charset=UTF-8`, 'Content-Transfer-Encoding: 8bit', '', body].join('\r\n')
